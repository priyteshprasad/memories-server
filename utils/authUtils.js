...
export const sendEmailVerificationMail = (email, token) => {
    const transporter = nodemailer.createTransport({
      host: "smtp.gmail.com",
      port: 465, //secure port
      secure: true, // to avoid going to spam folder, but it can still go to spam from localhost server
      service: "gmail",
      auth: {
        user: process.env.NODEMAILER_EMAIL,
        pass: process.env.NODEMAILER_PASS, //temp password
      },
    });
  
    const mailOptions = {
      from: "priyteshprasad@gmail.com",
      to: email,
      subject: "Email Verification for Memories App",
      html: `<head>
                  ...
                  <td bgcolor="#ffffff" style="padding: 40px 30px 40px 30px;">
                    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse: collapse;">
                      ...
                      <tr>
                        ...
                        <a href="http://localhost:8000/user/verify/${token}" style=" border: none;
    ...
    transporter.sendMail(mailOptions, function (error, info) {
      if (error) {
        ...
        console.log(`Email has been sent successfully: ${email}`);
      }
    });
